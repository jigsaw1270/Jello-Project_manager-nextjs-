...
const Sidebar = () => {
  ...
  const [showPriority, setShowPriorities] = useState(true);

  const dispatch =  useAppDispatch();
const isSidebarCollapsed = useAppSelector((state) => state.global.isSIdebarCollapsed,);

  const sidebarClassNames = ` fixed flex flex-xol  h-[100%] justify-between shadow-xl  transition-all  duration-300  h-full z-40 dark:bg-black  overflow-y-auto bg-white ${
  ...
  return (
    <div className={sidebarClassNames}>
      <div className="flex h-[100%] w-full flex-col justify-start">
        ...
        {/* team */}
        <div className="items-center gap-5 flex border-y-[1.5px] border-gray-200 px-8 py-4 dark:border-gray-700">
...
{/* 
 ...
 {/* priorities links */}
<button onClick={() => setShowPriorities((prev)=> !prev)} className="flex w-full items-center justify-between px-8 py-3 text-gray-500">
...
return (
  <Link href={href} className="w-full">
    <div className={`relative flex cursor-pointer items-center gap-3 transition-colors hover:bg-gray-100 dark:bg-black dark:hover:bg-gray-700 ${
      ...
      <Icon className="j-6 w-6 text-gray-800 dark:text-gray-100" />
